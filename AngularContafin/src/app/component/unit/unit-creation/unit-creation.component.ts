...
export class UnitCreationComponent implements OnInit {
  ...
  constructor(private router: Router, private unitService: UnitsService, public formBuilder: FormBuilder, private actRoute: ActivatedRoute, private sanitizer: DomSanitizer, private errorService :ErrorService) {
  }
...
public addLesson(lesson : Lesson): void{
  ...
  if(lesson !== null){
    this.idsPrevius.push(lesson.id);
...
public  transformToPair(text1: string, text2: string, id) : any{
  let arrPair = new Array();
  if(text1 === "" && text2 === ""){
    for(let i = 0; i < 5; i++){
      this.createUnitForm.addControl('pair_'+i+'_left_'+id, new FormControl(""));
      this.createUnitForm.addControl('pair_'+i+'_right_'+id, new FormControl(""));
      arrPair.push({left : "izquierda", right : "derecha"});
    }
  }else{
    let leftoptions = text1.split(":").map((element: string) => {
      return element.trim();
    });
  
    let rightoptions = text2.split(":").map((element: string) => {
      return element.trim();
    })
  
    for(let i = 0; i < 5; i++){
      arrPair.push({left : leftoptions[i], right : rightoptions[i]});
      this.createUnitForm.addControl('pair_'+i+'_left_'+id, new FormControl(leftoptions[i]));
      this.createUnitForm.addControl('pair_'+i+'_right_'+id, new FormControl(rightoptions[i]));
    }
  }
...
public addExercise(kind, exercise : Exercise, lessonId): number{
  let id = this.getId();
  console.log("Tipo del ejercicio creado: " + kind);

  if(exercise !== null){
    this.idsPrevius.push(exercise.id);
    ...
    if(exercise.kind === 3){
      //Añadir conertidor
      this.pair[id] = this.transformToPair(exercise.texts[0],exercise.texts[1], id);
    ...
    if(kind === 3){
      this.pair[id] = this.transformToPair("","", id);
    }
...
public preSave(){
  ...
  this.lessons.forEach((lesson)=>{
    ...
    lesson.exerciseList.forEach((exercise)=>{
      ...
      if(this.exercises[exercise] !== 2 && this.exercises[exercise] !== 3){
      ...
      }else if(this.exercises[exercise] === 3){
        let elementLeft;
        let elementRight;
        let index = 0;
        texts1 = "";
        texts2 = "";
        texts3 = "";
        this.pair[exercise].forEach(()=>{
              elementLeft = FORM_CONTROL['pair_'+index+'_left_'+exercise].value;
              elementRight = FORM_CONTROL['pair_'+index+'_right_'+exercise].value;
          if(typeof elementLeft !== 'undefined' && elementLeft !== null && elementLeft !== '' && typeof elementRight !== 'undefined' && elementRight !== null && elementRight !== '' ){
            if(texts1 !== ""){
              texts1 = texts1+":"+elementLeft;
              texts2 = texts2+":"+elementRight;
              result.value = result.value+"/"+elementLeft+":"+elementRight;
            }else{
              texts1 = elementLeft;
              texts2 = elementRight;
              result.value = elementLeft+":"+elementRight;
            }
          }else{
            this.alertDanger = true;
            console.log("666");
        ...
        console.log("Texto 1:" + texts1);
        console.log("Texto 2:" + texts2);
      }else{
        texts1 = '';
        texts2 = '';
        texts3 = '';
      ...
      if(!this.alertDanger){
        ...
        if(this.unitId !== -1 || this.idsPrevius.includes(exercise)){
          data.id = exercise;
        }
    ...
    if(!this.alertDanger){
      ...
      if(this.unitId !== -1 || this.idsPrevius.includes(lesson.id)){
        data.id = lesson.id;
      }
  ...
  selectFile(event, id, order) {
    ...
    reader.onloadend = (e) => {
      if(order === 1){
        this.imagesPreview1[id]= reader.result.toString();
      }
      else if(order === 2){
        this.imagesPreview2[id]= reader.result.toString();
      }else{
        this.imagesPreview3[id]= reader.result.toString();
      }
      FORM_CONTROL['images_'+order+'_'+id].setValue(reader.result.toString().split('base64,')[1]);
